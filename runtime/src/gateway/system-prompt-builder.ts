...
export function buildDesktopContext(
  ...
  if (desktopEnabled && !isMac && environment === "both") {
    ctx +=
      ...
      '- For coding workflows, prefer the native tools first: system.grep, system.glob, system.git*, system.readFile (use offset/limit for targeted reads), system.applyPatch, and system.symbol*. Use system.repoInventory only when you specifically need repo/worktree inventory.\n' +
...
export async function buildBaseSystemPrompt(
  ...
  const planningInstruction = options?.forVoice
    ...
    : "\n\n## Task Execution Protocol\n\n" +
      ...
      "6. If you materially changed workspace files as implementation work, do not self-certify completion. Wait for independent verifier confirmation before claiming the implementation is done. If verification cannot be completed, say that explicitly instead of declaring success.\n" +
      "7. Finish with grounded results or a specific blocker backed by the tool evidence\n" +
      "8. NEVER run interactive programs (games, TUI apps, editors, REPLs) via system.bash — they block the terminal. To test a GUI/TUI program, just compile it and confirm the binary exists\n\n" +
